...
pi = np.pi
quantization_level = 16 # bits per sample # to be chosen by the user
""" Define transmitted signal """
N=100               # Number of bits , size of transmitted signal x_inp=[x_0 x_2 ... x_(N-1)] 
x_inp = np.random.randint(0,2,(1,N))        #   binary signal 0 or 1  message to be transmitted
...
# f1 = figure(1);
# set(f1,'color',[1 1 1]);
fig,(input, bpsk) = plt.subplots(2,1)
...
# pdb.set_trace()
bunu_yaz = np.int16(x_mod/np.max(np.abs(x_mod)) * 2**(quantization_level-1))
